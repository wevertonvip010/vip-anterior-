...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, Clock, User, MapPin, Truck, Archive } from 'lucide-react';
...
const CalendarioDashboard = () => {
  ...
  const obterAtividadesDoDia = (dia) => {
    ...
    return atividades.filter(atividade => atividade.data === dataFormatada);
  };
        ...
        <div className="grid grid-cols-4 gap-4 text-center">
          <div>
            <div className="text-lg font-bold text-green-600">
              {atividades.filter(a => a.tipo === 'mudanca' && a.data.startsWith(`${mesAtual.getFullYear()}-${String(mesAtual.getMonth() + 1).padStart(2, '0')}`)).length}
            </div>
            <div className="text-xs text-gray-600">Mudanças</div>
          </div>
          <div>
            <div className="text-lg font-bold text-red-600">
              {atividades.filter(a => a.tipo === 'visita' && a.data.startsWith(`${mesAtual.getFullYear()}-${String(mesAtual.getMonth() + 1).padStart(2, '0')}`)).length}
            </div>
            <div className="text-xs text-gray-600">Visitas</div>
          </div>
          <div>
            <div className="text-lg font-bold text-yellow-600">
              {atividades.filter(a => a.tipo === 'entrada_box' && a.data.startsWith(`${mesAtual.getFullYear()}-${String(mesAtual.getMonth() + 1).padStart(2, '0')}`)).length}
            </div>
            <div className="text-xs text-gray-600">Entradas</div>
          </div>
          <div>
            <div className="text-lg font-bold text-orange-600">
              {atividades.filter(a => a.tipo === 'saida_box' && a.data.startsWith(`${mesAtual.getFullYear()}-${String(mesAtual.getMonth() + 1).padStart(2, '0')}`)).length}
            </div>
